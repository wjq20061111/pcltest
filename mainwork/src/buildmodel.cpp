...
int main (int argc, char** argv)
{
	...
	for(int fi=0;fi<14;fi++)
	{
  	...
  	// Mandatory
		...
		passthroughfilter(cloud_plane,cloud_plane,'y',centroid[1]-0.2,centroid[1]+0.0);
	...
	// Creating the KdTree object for the search method of the extraction
		...
		extractinliers(cloud_plane,
			boost::shared_ptr<pcl::PointIndices>(new pcl::PointIndices( cluster_indices[0] ) ),
			target);

		pcl::PointCloud<PointT>::Ptr cloud_centroid (new pcl::PointCloud<PointT>);		
		pcl::compute3DCentroid (*target, centroid);
		pcl::demeanPointCloud<PointT> (*target, centroid, *cloud_centroid);
//pclviewer(cloud_centroid);
		...
		std::cout<<"down "<<fi<<" capture"<<std::endl;
	}
